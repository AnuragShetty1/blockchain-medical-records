...
export default function VerifiedUploadForm({ patientProfile, onUploadSuccess, allowedCategories }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!file || !title || !category) return toast.error("Please provide a file, title, and category.");
        if (!contract || !professionalProfile?.publicKey || !patientProfile?.publicKey) return toast.error("User profiles not fully loaded. Cannot encrypt.");
        ...
        try {
            ...
            const metadata = { title, category, fileType: file.type || 'application/octet-stream', fileName: file.name, encryptedBundleIPFSHash: bundleHash };
            ...
            if (!patientKeyBundle || !professionalKeyBundle) throw new Error("Encryption failed: Could not create keys.");
            ...
            const tx = await contract.addVerifiedRecord(patientProfile.walletAddress, metadataHash, title, category, patientKeyBytes, professionalKeyBytes);
            await tx.wait();
    ...
    return (
        <div className="bg-white p-6 border border-slate-200 rounded-lg">
            <form onSubmit={handleSubmit} className="space-y-6">
                <fieldset>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Annual Blood Test Results" className="w-full px-3 py-2 border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500" required />
                        </div>
                        <div>
                ...
                <fieldset>
                    <legend className="text-sm font-semibold text-slate-600 border-b border-slate-200 pb-2 mb-4 w-full">2. Attach File</legend>
                    {!file ? (
                        <div onDrop={handleDrop} onDragOver={handleDrag} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} className={`relative block w-full px-6 py-8 text-center border-2 border-dashed rounded-lg cursor-pointer ${isDragActive ? 'border-sky-500 bg-sky-50/50' : 'border-slate-300 hover:border-slate-400'} transition-colors`}>
                            ...
                            <span className="mt-2 block text-sm font-medium text-slate-700">Drag & drop or <span className="text-sky-600 font-semibold">browse</span></span>
                        ...
                        <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg">
                            ...
                            <button type="button" onClick={() => setFile(null)} className="p-1.5 text-slate-500 rounded-full hover:bg-slate-200 hover:text-slate-800 transition-colors flex-shrink-0"><CloseIcon className="h-5 w-5"/></button>
                        </div>
                    )}
                </fieldset>
                ...
                <div className="pt-4">
                    <button type="submit" disabled={isUploading || !file || !title} className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 font-semibold text-white bg-sky-600 rounded-lg hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors shadow-sm">
                        {isUploading ? <><Loader2 className="h-5 w-5 animate-spin" /> Processing...</> : <>Encrypt & Upload Record</>}
                    </button>
                </div>
            </form>
        </div>
