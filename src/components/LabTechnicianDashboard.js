        ...
        <div className="w-full min-h-[calc(100vh-128px)] bg-slate-50">
             <header className="bg-white shadow-sm border-b border-slate-200">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <h1 className="text-3xl font-bold text-slate-900">Lab Technician's Dashboard</h1>
                    <p className="mt-1 text-slate-600">Welcome back, {userProfile?.name}!</p>
                </div>
            </header>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                 <div className="border-b border-slate-200 mb-8">
                    <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                        ...
                        <TabButton id="upload" label="Upload Lab Result" icon={<UploadIcon />} activeTab={activeTab} setActiveTab={setActiveTab} />
                    </nav>
                </div>
    ...
    const fetchSharedRecords = useCallback(async () => {
        ...
        try {
            ...
            const recordsWithMetadataPromises = events.map(async (event) => {
                ...
                let metadata = {};
                try {
                    const response = await fetch(`${IPFS_GATEWAY}/ipfs/${record.ipfsHash}`);
                    if (response.ok) {
