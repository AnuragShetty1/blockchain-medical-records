...
// NEW: Thematic icons for lab technician
import { Beaker, UploadCloud, FileSearch, HeartHandshake, Search, User, X as CloseIcon, ChevronDown, Info, Loader2 } from 'lucide-react';

// --- STATIC "MODERN CLINICAL" BACKGROUND (Consistent across dashboards) ---
...
// --- REDESIGNED WELCOME HEADER (Lab Themed) ---
const LabHeader = ({ labProfile }) => (
    <div className="mb-8">
        {!labProfile ? (
    ...
    return (
        <div className="relative w-full min-h-[calc(100vh-128px)]">
            ...
            <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <LabHeader labProfile={userProfile} />
...
const TabButton = ({ id, label, icon, activeTab, setActiveTab }) => (
    <button
        ...
        className={`${
            activeTab === id
                ? 'border-emerald-600 text-emerald-600' // Green accent
                : 'border-transparent text-slate-500 hover:text-slate-800 hover:border-slate-300'
        } whitespace-nowrap py-4 px-1 border-b-2 font-semibold text-base inline-flex items-center gap-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500`}
    >
        {icon}
        {label}
    </button>
        ...
        <div className="border border-slate-200 rounded-lg bg-white p-4 h-full">
            ...
            <div className="relative mb-3">
                ...
                <input
                    ...
                    className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 transition" // Green accent
                />
                    ...
                    <div className="space-y-4">
                        <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200 flex justify-between items-center">
                            <div>
                                ...
                                <p className="font-bold text-emerald-900">{patientProfile.name}</p>
                            </div>
                            <button onClick={() => setPatientProfile(null)} className="text-sm font-semibold text-emerald-600 hover:text-emerald-800 flex items-center gap-1">
            ...
            <div className="lg:col-span-2">
                {patientProfile ? (
                    <div className="space-y-4">
                        <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200 flex justify-between items-center">
                            <div>
                                <p className="text-sm text-emerald-700">Requesting access for:</p>
                                <p className="font-bold text-emerald-900">{patientProfile.name}</p>
                            </div>
                            <button onClick={() => setPatientProfile(null)} className="text-sm font-semibold text-emerald-600 hover:text-emerald-800 flex items-center gap-1">
                                <CloseIcon className="h-4 w-4" /> Clear
    ...
    return (
        <div className="bg-white rounded-xl shadow-md border border-slate-200/80 overflow-hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center p-4 text-left hover:bg-slate-50/50 transition-colors">
                ...
                <div className="flex items-center gap-4">
                    <span className="text-sm font-medium bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full">{records.length} Record(s)</span>
